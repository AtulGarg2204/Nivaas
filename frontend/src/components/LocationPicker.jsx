...
const LocationPicker = () => {
  ...
  const updateCardsPerView = () => {
    if (window.innerWidth < 640) {
      setCardsPerView(2); // Mobile: UPDATED to 2 cards
  ...
  return (
    <div className="py-16 bg-white border-b border-gray-100">
      <div className="container mx-auto px-4 sm:px-6 md:px-8 lg:px-16">
        ...
        <div className="relative" style={{ overflow: 'hidden' }}>
          {/* Left Arrow Button - Positioned in existing margin */}
          <button 
            onClick={() => scroll('left')}
            className={`absolute -left-4 top-1/2 -translate-y-1/2 z-10 bg-white/90 rounded-full p-2 shadow-lg text-gray-800 transition-colors duration-300 focus:outline-none ${activeIndex === 0 ? 'opacity-50 cursor-not-allowed' : 'opacity-100 hover:bg-gray-100'}`}
            aria-label="Scroll left"
            disabled={activeIndex === 0}
          >
            <svg xmlns="http://www.w3.org/2000/svg" className="h-6 w-6" fill="none" viewBox="0 0 24 24" stroke="currentColor">
          ...
          {/* Right Arrow Button - Positioned in existing margin */}
          <button 
            onClick={() => scroll('right')}
            className={`absolute -right-4 top-1/2 -translate-y-1/2 z-10 bg-white/90 rounded-full p-2 shadow-lg text-gray-800 transition-colors duration-300 focus:outline-none ${activeIndex >= maxScrollIndex ? 'opacity-50 cursor-not-allowed' : 'opacity-100 hover:bg-gray-100'}`}
            aria-label="Scroll right"
